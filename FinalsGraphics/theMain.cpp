...
cLight* pSpotLightForSHark = new cLight();
...
glm::vec2 waterOffset = glm::vec2(0.0f, 0.0f);
...
//			pBall->diffuseColour = glm::vec4(1.0f, 0.0f, 0.0f, 0.0f);
	...
	if (isShiftKeyDownByAlone(mods))
	{
		...
		if (key == GLFW_KEY_A)
		{
			pLightsVec.at(2)->_PositionX -= CAMERASPEED;
		}
		if (key == GLFW_KEY_D)
		{
			pLightsVec.at(2)->_PositionX += CAMERASPEED;
		...
		if (key == GLFW_KEY_Q)
		{
			pLightsVec.at(2)->_PositionY -= CAMERASPEED;
		}
		if (key == GLFW_KEY_E)
		{
			pLightsVec.at(2)->_PositionY += CAMERASPEED;
		...
		if (key == GLFW_KEY_W)
		{
			pLightsVec.at(2)->_PositionZ -= CAMERASPEED;
		}
		if (key == GLFW_KEY_S)
		{
			pLightsVec.at(2)->_PositionZ += CAMERASPEED;
...
int main(void)
{
	...
	cMesh IslandMesh;
	pTheModelLoader->LoadPlyModel("assets/models/IslandTerrain_XYZ_scaled_n_tex.ply", IslandMesh);

	cMesh SeaFloorMesh;
	pTheModelLoader->LoadPlyModel("assets/models/SeaFloorTerrain_XYZ_n_tex.ply", SeaFloorMesh);

	cMesh FlatWaterMesh;
	pTheModelLoader->LoadPlyModel("assets/models/FlatWaterTerrain_XYZ_n_tex.ply", FlatWaterMesh);
	...
	cMesh fishMesh;
	pTheModelLoader->LoadPlyModel("assets/models/fish_xyz_n_uv_scaled.ply", fishMesh);

	cMesh sharkMesh;
	pTheModelLoader->LoadPlyModel("assets/models/shark_xyz_n_uv_scaled.ply", sharkMesh);
	...
	sModelDrawInfo IslandMeshInfo;
	pTheVAOManager->LoadModelIntoVAO("island", IslandMesh, IslandMeshInfo, shaderProgID);

	sModelDrawInfo SeaFloorMeshInfo;
	pTheVAOManager->LoadModelIntoVAO("seaFloor", SeaFloorMesh, SeaFloorMeshInfo, shaderProgID);

	sModelDrawInfo FlatWaterMeshInfo;
	pTheVAOManager->LoadModelIntoVAO("flatWater", FlatWaterMesh, FlatWaterMeshInfo, shaderProgID);
	...
	::g_pTextureManager->Create2DTextureFromBMPFile("sandTex_1024.bmp", true);
	::g_pTextureManager->Create2DTextureFromBMPFile("orangeSand_512.bmp", true);
	...
	::g_pTextureManager->Create2DTextureFromBMPFile("water_tex_512.bmp", true);
	::g_pTextureManager->Create2DTextureFromBMPFile("realGrass_512.bmp", true);

	::g_pTextureManager->Create2DTextureFromBMPFile("FishTex_512.bmp", true);
	::g_pTextureManager->Create2DTextureFromBMPFile("sharkTex_512.bmp", true);
	...
	//Second cube map
	if (::g_pTextureManager->CreateCubeTextureFromBMPFiles("space",
		"alpha-island_rt.bmp", "alpha-island_lf.bmp",
		"alpha-island_up.bmp", "alpha-island_dn.bmp",
		"alpha-island_ft.bmp", "alpha-island_bk.bmp", true, errorString))
	{
		std::cout << "space skybox loaded" << std::endl;
	}
	else
	...
	iObject* pIsland = pFactory->CreateObject("mesh");
	pIsland->setMeshName("island");
	pIsland->setFriendlyName("island");	// We use to search 
	pIsland->setPositionXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pIsland->setRotationXYZ(glm::quat(glm::vec3(0.0f, 0.0f, 0.0f)));
	pIsland->setScale(1.0f);
	pIsland->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pSphere->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pIsland->setInverseMass(0.0f);
	pIsland->setIsVisible(true);
	pIsland->setIsWireframe(false);
	pIsland->setTexture("StoneTex_1024.bmp", 0); 
	pIsland->setTexture("realGrass_512.bmp", 1);
	pIsland->setTexture("sandTexture_512.bmp", 2);
	pIsland->setTextureRatio(1, 0);
	pIsland->setTextureRatio(1, 1);
	pIsland->setTextureRatio(1, 2);
	pIsland->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pIsland);

	iObject* pSeaFloor = pFactory->CreateObject("mesh");
	pSeaFloor->setMeshName("seaFloor");
	pSeaFloor->setFriendlyName("seaFloor");	// We use to search 
	pSeaFloor->setPositionXYZ(glm::vec3(0.0f,0.1f, 0.0f));
	pSeaFloor->setRotationXYZ(glm::quat(glm::vec3(0.0f, 0.0f, 0.0f)));
	pSeaFloor->setScale(2.0f);
	pSeaFloor->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pSphere->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pSeaFloor->setInverseMass(0.0f);
	pSeaFloor->setIsVisible(true);
	pSeaFloor->setIsWireframe(false);
	pSeaFloor->setTexture("orangeSand_512.bmp", 0);
	pSeaFloor->setTexture("water_tex_512.bmp", 1);
	pSeaFloor->setTextureRatio(0.4, 0);
	pSeaFloor->setTextureRatio(0.6, 1);
	pSeaFloor->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pSeaFloor);

	iObject* pShark = pFactory->CreateObject("mesh");
	pShark->setMeshName("shark");
	pShark->setFriendlyName("shark");	// We use to search 
	pShark->setPositionXYZ(glm::vec3(-160, 20, -120));
	pShark->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pShark->setScale(0.1f);
	pShark->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pSharkerge->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pShark->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pShark->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pShark->setInverseMass(0.0f);
	pShark->setIsVisible(true);
	pShark->setIsWireframe(false);
	pShark->setTexture("sharkTex_512.bmp", 0);
	pShark->setTextureRatio(1, 0);
	pShark->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pShark);

	iObject* pFish1 = pFactory->CreateObject("mesh");
	pFish1->setMeshName("fish");
	pFish1->setFriendlyName("fish1");	// We use to search 
	pFish1->setPositionXYZ(glm::vec3(-180, 20, -60));
	pFish1->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish1->setScale(0.1f);
	pFish1->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pFi1shrkerge->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pFish1->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish1->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish1->setInverseMass(0.0f);
	pFish1->setIsVisible(true);
	pFish1->setIsWireframe(false);
	pFish1->setTexture("FishTex_512.bmp", 0);
	pFish1->setTextureRatio(1, 0);
	pFish1->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pFish1);

	iObject* pFish2 = pFactory->CreateObject("mesh");
	pFish2->setMeshName("fish");
	pFish2->setFriendlyName("fish2");	// We use to search 
	pFish2->setPositionXYZ(glm::vec3(-160, 20, -40));
	pFish2->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish2->setScale(0.1f);
	pFish2->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pFi2shrkerge->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pFish2->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish2->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish2->setInverseMass(0.0f);
	pFish2->setIsVisible(true);
	pFish2->setIsWireframe(false);
	pFish2->setTexture("FishTex_512.bmp", 0);
	pFish2->setTextureRatio(1, 0);
	pFish2->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pFish2);

	iObject* pFish3 = pFactory->CreateObject("mesh");
	pFish3->setMeshName("fish");
	pFish3->setFriendlyName("fish3");	// We use to search 
	pFish3->setPositionXYZ(glm::vec3(-140, 20, -60));
	pFish3->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish3->setScale(0.1f);
	pFish3->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pFi3shrkerge->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pFish3->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish3->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish3->setInverseMass(0.0f);
	pFish3->setIsVisible(true);
	pFish3->setIsWireframe(false);
	pFish3->setTexture("FishTex_512.bmp", 0);
	pFish3->setTextureRatio(1, 0);
	pFish3->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pFish3);

	iObject* pFish4 = pFactory->CreateObject("mesh");
	pFish4->setMeshName("fish");
	pFish4->setFriendlyName("fish4");	// We use to search 
	pFish4->setPositionXYZ(glm::vec3(-120, 20, -80));
	pFish4->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish4->setScale(0.1f);
	pFish4->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pFi4shrkerge->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pFish4->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish4->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish4->setInverseMass(0.0f);
	pFish4->setIsVisible(true);
	pFish4->setIsWireframe(false);
	pFish4->setTexture("FishTex_512.bmp", 0);
	pFish4->setTextureRatio(1, 0);
	pFish4->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pFish4);

	iObject* pFish5 = pFactory->CreateObject("mesh");
	pFish5->setMeshName("fish");
	pFish5->setFriendlyName("fish5");	// We use to search 
	pFish5->setPositionXYZ(glm::vec3(-160, 20, -80));
	pFish5->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish5->setScale(0.1f);
	pFish5->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pFi5shrkerge->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pFish5->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish5->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pFish5->setInverseMass(0.0f);
	pFish5->setIsVisible(true);
	pFish5->setIsWireframe(false);
	pFish5->setTexture("FishTex_512.bmp", 0);
	pFish5->setTextureRatio(1, 0);
	pFish5->setTransprancyValue(1.0f);
	::g_vec_pGameObjects.push_back(pFish5);

	iObject* pFlatWater = pFactory->CreateObject("mesh");
	pFlatWater->setMeshName("flatWater");
	pFlatWater->setFriendlyName("flatWater");	// We use to search 
	pFlatWater->setPositionXYZ(glm::vec3(0.0f, 20.0f, 0.0f));
	pFlatWater->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pFlatWater->setScale(2.0f);
	pFlatWater->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	//pFlatWaterge->setDebugColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pFlatWater->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pFlatWater->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pFlatWater->setInverseMass(0.0f);
	pFlatWater->setIsVisible(true);
	pFlatWater->setIsWireframe(false);
	pFlatWater->setTexture("water_tex_512.bmp", 0);
	pFlatWater->setTextureRatio(1, 0);  
	pFlatWater->setTransprancyValue(0.6f);
	::g_vec_pGameObjects.push_back(pFlatWater);
	...
	pSpotLightForSHark->setNodeName("SpotLight");
	xml_node spotLightNode = lightData.child("SpotLight");
	pSpotLightForSHark->setPositionX(std::stof(spotLightNode.child("PositionX").child_value()));
	pSpotLightForSHark->setPositionY(std::stof(spotLightNode.child("PositionY").child_value()));
	pSpotLightForSHark->setPositionZ(std::stof(spotLightNode.child("PositionZ").child_value()));
	pSpotLightForSHark->setPositionXYZ(glm::vec3(std::stof(spotLightNode.child("PositionX").child_value()), std::stof(mainLight1Node.child("PositionY").child_value()), std::stof(mainLight1Node.child("PositionZ").child_value())));
	pSpotLightForSHark->setConstAtten(std::stof(spotLightNode.child("ConstAtten").child_value()));
	pSpotLightForSHark->setLinearAtten(std::stof(spotLightNode.child("LinearAtten").child_value()));
	pSpotLightForSHark->setQuadraticAtten(std::stof(spotLightNode.child("QuadraticAtten").child_value()));
	pSpotLightForSHark->setInnerSpot(std::stof(spotLightNode.child("SpotInnerAngle").child_value()));
	pSpotLightForSHark->setOuterSpot(std::stof(spotLightNode.child("SpotOuterAngle").child_value()));

	pLightsVec.push_back(pMainLight);
	pLightsVec.push_back(pMainLight1);
	pLightsVec.push_back(pSpotLightForSHark);
	...
	::g_pFlyCamera->eye = glm::vec3(21, 99, -353);
...
// View matrix
		...
		for (lightIndex; lightIndex < pLightsVec.size(); ++lightIndex)
		{
			...
			if(lightIndex==2)
			{
				...
				glm::vec3 dirOfSpot = pShark->getPositionXYZ() - pLightsVec.at(2)->getPositionXYZ();

				glm::vec3 spotLightDir = glm::normalize(dirOfSpot);
				glUniform4f(direction, spotLightDir.x, spotLightDir.y, spotLightDir.z, 1.0f);
				glUniform4f(param1, 1.0f /*POINT light*/, pLightsVec.at(lightIndex)->getInnerSpot(), pLightsVec.at(lightIndex)->getOuterSpot(), 1.0f);
			}
			else
			{
		...
		glUniform4f(eyeLocation_UL,
			...
			ssTitle
				<< "The Tree is at: "
				/*<< ::g_vec_pEnvironmentObjects[changeObj]->getFriendlyName() << " = "
				...
				<< ::g_pFlyCamera->eye.z << ", "
				<< "eagle is: "
				<< pLightsVec.at(2)->_PositionX << ", "
				<< pLightsVec.at(2)->_PositionY << ", "
				<< pLightsVec.at(2)->_PositionZ << ", ";
...
void DrawObject(glm::mat4 m, iObject* pCurrentObject, GLint shaderProgID, cVAOManager* pVAOManager)
{
	...
	{
		GLint isIsland = glGetUniformLocation(shaderProgID, "isIsland");
		glUniform1f(isIsland, (float)GL_TRUE);
	}
	else
	{
		GLint isIsland = glGetUniformLocation(shaderProgID, "isIsland");
		glUniform1f(isIsland, (float)GL_FALSE);
	}

	if (pCurrentObject->getFriendlyName() == "flatWater")
	{
		GLint isIsland = glGetUniformLocation(shaderProgID, "isWater");
		glUniform1f(isIsland, (float)GL_TRUE);
	}
	else
	{
	...
	if (pCurrentObject->getFriendlyName() != "skybox")
	{
		...
		if (pCurrentObject->getFriendlyName() == "flatWater")
		{
			...
			GLint useHeightMap_UL = glGetUniformLocation(shaderProgID, "waterHeight");
	...
	//		GLuint heightMapID = ::g_pTextureManager->getTextureIDFromName("IslandHeightMap.bmp");
			GLuint heightMapID = ::g_pTextureManager->getTextureIDFromName("water_tex_512.bmp");
			...
			// Tie the texture units to the samplers in the shader
			GLint heightMap_UL = glGetUniformLocation(shaderProgID, "waterSampler");
			glUniform1i(heightMap_UL, TEXTURE_UNIT_40);	// Texture unit 18

			// ANOTHER HACK
			GLint textOffset_UL = glGetUniformLocation(shaderProgID, "waterOffset");
			glUniform2f(textOffset_UL, waterOffset.x, waterOffset.y);

			waterOffset.x += 0.1f;
			waterOffset.y += 0.017f;
		}
		else
		{
			GLint useHeightMap_UL = glGetUniformLocation(shaderProgID, "waterHeight");
